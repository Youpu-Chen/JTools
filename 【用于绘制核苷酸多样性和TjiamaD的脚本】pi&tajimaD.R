...
# dat3 <- read.delim('passmon/1-3-passmon.NONV-SNPs-4D-CladeE-W1.WPM.txt', sep = '\t')
...
# dat7 <- read.delim('passmon/2-3-passmon.NONV-SNPs-all-CladeE-W1.WPM.txt', sep = '\t')
...
fourD_sites[which(fourD_sites$pop == "CladeEallin"), ]$pop <- "CladeE"
fourD_sites[which(fourD_sites$pop == "CladeW1allin"), ]$pop <- "CladeW1"

fourD_sites$pop <- factor(fourD_sites$pop, levels = c('CladeE', 'CladeW1', 'CladeW2'))
...
all_sites[which(all_sites$pop == "CladeEall"), ]$pop <- "CladeE"
all_sites[which(all_sites$pop == "CladeW1all"), ]$pop <- "CladeW1"

all_sites$pop <- factor(all_sites$pop, levels = c('CladeE', 'CladeW1', 'CladeW2'))
...
if(F){
  ...
  theme_classic() +
  geom_signif(comparisons = list(c("CladeE", "CladeW1"),
                                 c("CladeE", "CladeW2"),
                                 c("CladeW1", "CladeW2")),
              map_signif_level=FALSE,
              textsize=5,
  ...
  xlab(NULL)
  p1
  ggsave('2022-5-23-significance-4D_TajimaD.png', p1, width = 6, height = 6, dpi = 600)
}
...
if(F){
  p2 <- ggplot(data = fourD_sites, aes(x = pop, y = Diversity)) + 
  # geom_violin() + 
  geom_boxplot(width=0.7, lwd = 0.5, outlier.size = 3, outlier.alpha = 0.3) + 
  theme_classic() +
  geom_signif(comparisons = list(c("CladeE", "CladeW1"),
                                 c("CladeE", "CladeW2"),
                                 c("CladeW1", "CladeW2")),
  ...
  xlab(NULL)
  p2
  ggsave('2022-5-23-significance-4D_Diversity.png', p2, width = 6, height = 6, dpi = 600)
}
...
if(F){
  ...
  xlab(NULL)
  p3
  ggsave('2022-5-23-significance-all_TajimaD.png', p3, width = 6, height = 6, dpi = 600)
...
if(F){
  ...
  xlab(NULL)
  p4
  ggsave('2022-5-23-significance-all_Diversity.png', p4, width = 6, height = 6, dpi = 600)
...
#-------------------------------------------------------------------------------
# fourD_summary_df <- as.data.frame(fourD_sites %>% group_by(pop) %>% summarise(TajimaD_mean = mean(D),
#                                                                               TajimaD_SD = sd(D),
...
library(openxlsx)
# sheets = list("sheet1"=fourD_summary_df,
#               "sheet2"=all_summary_df)
