...
test.select <- AnnotationDbi::select(org.Sc.sgd.db, keys = counts$Geneid[1:500], 
                      columns=c('GENENAME'), 
                      keytype="ENSEMBL")
which('peptile' %in% test.select$DESCRIPTION)
...
cellCycleGO <- names(GO_DATA$PATHID2NAME[grep("peptide metabolic process", GO_DATA$PATHID2NAME)])
# [1] "GO:0006518" "GO:0050761"
...
length(unique(counts$Geneid[1:500][counts$Geneid[1:500] %in% cellCycleGene]))
