        ...
        if local == False:
            ...
            for i in range(1, len(m_mat)):
                for j in range(1, len(m_mat[0])):
                    m_mat[i][j] = max(
                        m_mat[i-1][j-1] + identity_match(seq1[i-1], seq2[j-1], mat, mism),
                        x_mat[i-1][j-1] + identity_match(seq1[i-1], seq2[j-1], mat, mism),
                        y_mat[i-1][j-1] + identity_match(seq1[i-1], seq2[j-1], mat, mism)
                    )
...
def Runalignment(seq1, seq2, m, mism, g_o, g_e, local):
    ...
    if local == False:
        alignment = SequenceAlignment(seq1, seq2, m, mism, g_o, g_e, local=False)
    ...
    if local == True:
        alignment = SequenceAlignment(seq1, seq2, m, mism, g_o, g_e, local=True)
        m_mat, x_mat, y_mat, trace_matrix = alignment.createscorematrix()
        m_mat, x_mat, y_mat, trace_matrix = alignment.matrix_init(m_mat, x_mat, y_mat, trace_matrix)
        score_matrix, trace_matrix = alignment.matrix_filling(m_mat, x_mat, y_mat, trace_matrix)
        # print(score_matrix)
